...
export class Data1000000005000 implements MigrationInterface {
    async up(queryRunner: QueryRunner): Promise<any> {
        await this.insertAccesses(queryRunner);
        await this.insertRoles(queryRunner);
        await this.insertRoleChildren(queryRunner);
        await this.insertRoleAccess(queryRunner);
        await this.insertAdminUser(queryRunner);
    }

    private async insertAccesses(queryRunner: QueryRunner) {
        ...
        if (Helper.isNull(accesses['7']))
            await queryRunner.query(
                "INSERT INTO `access` VALUES (7,'2019-06-04 16:51:17','2019-06-04 16:51:17',2,0,'events','Add/Modify/Delete Events');"
            );
        if (Helper.isNull(accesses['8']))
            await queryRunner.query(
                "INSERT INTO `access` VALUES (8,'2019-06-04 16:51:17','2019-06-04 16:51:17',2,0,'organisers','Add/Modify/Delete Organisers');"
            );
        if (Helper.isNull(accesses['9']))
            await queryRunner.query(
                "INSERT INTO `access` VALUES (9,'2019-06-04 16:51:17','2019-06-04 16:51:17',2,0,'fsjs','Add/Modify/Delete FSJ');"
            );
        if (Helper.isNull(accesses['10']))
            await queryRunner.query(
                "INSERT INTO `access` VALUES (10,'2019-06-04 16:51:17','2019-06-04 16:51:17',2,0,'posts','Add/Modify/Delete Posts');"
            );
    }
    ...
    private async insertRoles(queryRunner: QueryRunner) {
        ...
        if (Helper.isNull(roles['6']))
            await queryRunner.query(
                "INSERT INTO `role` VALUES (6,'2019-06-04 16:51:18','2019-06-04 16:51:18',2,0,'Echo-Moderator','Role for Echo-Moderator (events and organisers)');"
            );
    }

    private async insertRoleChildren(queryRunner: QueryRunner) {
    ...
    private async insertRoleAccess(queryRunner: QueryRunner) {
    ...
    private async insertAdminUser(queryRunner: QueryRunner) {
        ...
        const adminUser = await queryRunner.query("SELECT * FROM user WHERE id = 1 AND username = 'admin'");
        ...
        if (adminUser.length === 0) {
            ...
            await queryRunner.query(
                `${
                    ...
                    "(1,'2019-06-04 16:51:18','2019-06-04 16:51:24',3,0,'admin','echo@silas.link','"
                }${pw}',1,0,'${salt}')`
            );
