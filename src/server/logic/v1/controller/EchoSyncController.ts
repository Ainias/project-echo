...
export class EchoSyncController extends SyncController {
    ...
    static async _syncEvent(lastSynced, offset) {
        ...
        queryBuilder = queryBuilder.leftJoinAndSelect("Event.repeatedEvent", "repeatedEvent")
            ...
            .andWhere(new Brackets(qb => {
                qb.orWhere("Event.endTime >= :maxEndTime", {maxEndTime: lastEventToShow})
                    .orWhere(new Brackets(qb => {
                        qb.andWhere("Event.isTemplate = 1")
                            .andWhere(new Brackets(qb => {
                                qb.orWhere("repeatedEvent.repeatUntil >= :maxEndTime", {maxEndTime: lastEventToShow})
    ...
    static async _syncRepeatedEvent(lastSynced, offset) {
        ...
        let queryBuilder = <SelectQueryBuilder<RepeatedEvent>>await EasySyncServerDb.getInstance().createQueryBuilder(RepeatedEvent);
